<?php
/**
 * BuildTech CMS - Manifesto Slider Management
 * Manifesto slider yönetimi sayfası
 */
...
$page_title = 'Manifesto Slider Yönetimi';
$breadcrumb = [
    ['title' => 'Ana Sayfa', 'url' => 'dashboard.php'],
    ['title' => 'Manifesto Slider Yönetimi']
];
        ...
        switch ($action) {
            case 'add':
                ...
                if (empty($title)) {
                    ...
                    $sql = "INSERT INTO manifesto_sliders (title, subtitle, button_text, button_link, button_active, background_image, order_index, is_active) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    
                    if ($database->execute($sql, [$title, $subtitle, $button_text, $button_link, $button_active, $background_image, $order_index, $is_active])) {
                        setSuccessMessage('Manifesto slider başarıyla eklendi.');
                        header('Location: manifesto-sliders.php?action=list');
                        exit;
                    } else {
                        setErrorMessage('Manifesto slider eklenirken hata oluştu.');
                    }
                }
                break;
            ...
            case 'edit':
                ...
                if (empty($title)) {
                    ...
                    // Mevcut kaydı al
                    $slider = $database->fetchOne("SELECT * FROM manifesto_sliders WHERE id = ?", [$id]);
                    if (!$slider) {
                        setErrorMessage('Manifesto slider bulunamadı.');
                        break;
                    }
                    ...
                    $sql = "UPDATE manifesto_sliders SET title = ?, subtitle = ?, button_text = ?, button_link = ?, button_active = ?, 
                            background_image = ?, order_index = ?, is_active = ? WHERE id = ?";
                    
                    if ($database->execute($sql, [$title, $subtitle, $button_text, $button_link, $button_active, $background_image, $order_index, $is_active, $id])) {
                        setSuccessMessage('Manifesto slider başarıyla güncellendi.');
                        header('Location: manifesto-sliders.php?action=list');
                        exit;
                    } else {
                        setErrorMessage('Manifesto slider güncellenirken hata oluştu.');
                    }
                }
                break;
            ...
            case 'delete':
                ...
                if ($slider) {
                    ...
                    if ($database->execute("DELETE FROM manifesto_sliders WHERE id = ?", [$id])) {
                        setSuccessMessage('Manifesto slider başarıyla silindi.');
                    } else {
                        setErrorMessage('Manifesto slider silinirken hata oluştu.');
                    }
                } else {
                    setErrorMessage('Manifesto slider bulunamadı.');
                }
...
switch ($action) {
    case 'add':
        $breadcrumb[] = ['title' => 'Yeni Manifesto Slider'];
        break;
    case 'edit':
        $slider = $database->fetchOne("SELECT * FROM manifesto_sliders WHERE id = ?", [$id]);
        if (!$slider) {
        ...
        $breadcrumb[] = ['title' => 'Manifesto Slider Düzenle'];
        break;
    default:
        $sliders = $database->fetchAll("SELECT * FROM manifesto_sliders ORDER BY order_index ASC, created_at DESC");
        break;
}
...
<?php if ($action === 'list'): ?>
    <!-- Manifesto Slider Listesi -->
    <?= displayMessages() ?>
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="fas fa-images me-2"></i>Manifesto Slider Listesi</h5>
            <a href="manifesto-sliders.php?action=add" class="btn btn-primary">
                <i class="fas fa-plus me-2"></i>Yeni Manifesto Slider Ekle
            </a>
        </div>
        <div class="card-body">
            <?php if (empty($sliders)): ?>
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">Henüz manifesto slider bulunmuyor</h5>
                    <p class="text-muted">İlk manifesto slider'ınızı eklemek için yukarıdaki butona tıklayın.</p>
                    <a href="manifesto-sliders.php?action=add" class="btn btn-primary">
                        <i class="fas fa-plus me-2"></i>Yeni Manifesto Slider Ekle
                    </a>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="manifesto-sliders.php?action=edit&id=<?= $slider['id'] ?>" 
                                               class="btn btn-outline-primary" title="Düzenle">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="manifesto-sliders.php?action=delete&id=<?= $slider['id'] ?>" 
                                               class="btn btn-outline-danger" title="Sil"
                                               onclick="return confirmDelete('Bu manifesto slider\'ı silmek istediğinizden emin misiniz?')">
...
<?php else: ?>
    <!-- Manifesto Slider Ekleme/Düzenleme Formu -->
    <?= displayMessages() ?>
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">
                <i class="fas fa-<?= $action === 'add' ? 'plus' : 'edit' ?> me-2"></i>
                <?= $action === 'add' ? 'Yeni Manifesto Slider Ekle' : 'Manifesto Slider Düzenle' ?>
            </h5>
        </div>
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="button_link" 
                                           ...
                                           placeholder="#manifesto-content, #principles, vb.">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <div class="form-check form-switch">
                                        <input type="checkbox" class="form-check-input" name="button_active" 
                                               <?= ($slider['button_active'] ?? 1) ? 'checked' : '' ?>>
                                        <label class="form-check-label">Buton Aktif</label>
                                    </div>
                                    <small class="form-text text-muted">Butonun görünür olup olmayacağını belirler</small>
                                </div>
                            </div>
                        </div>
                ...
                <div class="d-flex justify-content-between">
                    <a href="manifesto-sliders.php" class="btn btn-secondary">
